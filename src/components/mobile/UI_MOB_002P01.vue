...
const popupHandler = ({ type = "search" || "mobileAlert" || "mobileConfirm", componentType = null, msg = null, props, ok, cancel }) => {
    popup({
        type,
        msg,
        componentType,
...
/** 그리드 속성 정의 */
const gridProps = AUIGridDefault.gridPropsBuilder().withExtraProps({ height: 480, noDataMessage: "출력할 데이터가 없습니다.", rowIdField: "itemCd", rowIdTrustMode: true }).build();
const columnLayout = [
    ...
    { dataField: "inQty", headerText: "입고수량", width: 53 },
    {
        ...
        headerText: "차이수량",
        width: 53,
        labelFunction: (rowIndex, columnIndex, value, headerText, item, dataField, cItem) => Number(item.inQty) - Number(item.ordQty),
    },
];
...
const barcodeScan = async (e) => {
    ...
    else if (result.errorCode == 4001) return (inputData.resultMsg = "협력사 지역별 바코드규칙정보가 존재하지 않습니다.");
    else if (result.errorCode == 4002) return (inputData.resultMsg = "외부품목코드 연결이 안됩니다.");
    ...
    if (!myGrid.value.getGridData().some((val) => val.itemCd == result.itemCd)) {
        ...
        myGrid.value.updateRowsById([{ itemCd: result.itemCd, inQty: Number(myGrid.value.getItemByRowId(result.itemCd)?.inQty) + Number(result.inQty) }]);
    }
};
                ...
                <li class="radioWrap">
                    ...
                    <div class="form_wrap">
                        <span class="form_cell form_radio">
                            <input type="radio" name="a1" id="radio01" checked @change="radioControl" />
                            <label for="radio01">
                                <span>개체</span>
                            </label>
                        </span>
                    ...
                    <div class="form_wrap">
                        <span class="form_cell form_radio" @change="radioControl">
                            <input type="radio" name="a1" id="radio02" />
                            <label for="radio02">
                                <span>개체 + 제품</span>
                            </label>
                        </span>
                    ...
                    <div class="form_wrap">
                        <span class="form_cell form_check login">
                            <input type="checkbox" id="check01" @change="removeCheck" />
        ...
        <table class="tableLayout mt20">
            ...
            <tbody>
                <tr>
                    <th>바코드</th>
                    <td>
                        <input type="text" style="border: 1px solid #ccc; outline: none" @keydown.enter="barcodeScan" autocomplete="off" id="barcodeInput" :inputmode="validate.keyboardFlag ? '' : 'none'" v-model="inputData.barcodeInput" />
                    </td>
                </tr>
                <tr>
                    <th>식별번호</th>
                    <td>
                        <input type="text" style="border: 1px solid #ccc; outline: none" @keydown.enter="distrHistoryScan" autocomplete="off" id="distrHistoryInput" :inputmode="validate.keyboardFlag ? '' : 'none'" v-model="inputData.distrHistoryInput" :disabled="!validate.barcodeFlag" />
                    </td>
                </tr>
                <tr>
                    <th>메세지</th>
